...
const store = useMemo<Store>(() => {
  ...
  return {
    ...
    setColor: (value: ColorValue) => {
      ...
      const prevFormat = stateRef.current.format;
      stateRef.current.color = value;
      if (propsRef.current.onValueChange) {
        propsRef.current.onValueChange(colorToString(value, prevFormat));
      }
      notify();
    },
    setHsv: (value: HSVColorValue) => {
      ...
      if (propsRef.current.onValueChange) {
        propsRef.current.onValueChange(
          colorToString(hsvToRgb(value), prevFormat),
        );
      }
      notify();
    },
...
function ColorPickerHueSlider(
  ...
  return (
    ...
    >
      ...
      <SliderPrimitive.Thumb className="block size-4 rounded-full border border-surface-border bg-text-primary shadow transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50" />
    </SliderPrimitive.Root>
  );
}
